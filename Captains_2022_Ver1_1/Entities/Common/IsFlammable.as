...
void onInit(CBlob@ this)
{
	...
	if (!this.exists(burn_duration))
		if (this.getName() == "ballista" || this.getName() == "catapult")
			this.set_s16(burn_duration , 9001);
		else
...
{
	if ((isIgniteHitter(customData)) ||					 	   // Fire arrows //damage > 0.0f
...
void onTick(CBlob@ this)
{
	...
	else if (burn_time > 0)
	{
		...
		if ((burn_time % 7) == 0)
		{
			...
			if (this.getName() == "ballista" || this.getName() == "catapult")
				blob.server_Hit(this, pos, Vec2f(0, 0), this.getInitialHealth()*0.1, this.get_u8(burn_hitter), true);
			else
