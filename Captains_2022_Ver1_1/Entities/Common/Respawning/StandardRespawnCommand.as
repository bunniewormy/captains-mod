...
void switch_class_on_number_keys (CBlob@ this, bool allow_enemies) {
    CBlob@ blob = getLocalPlayerBlob();
    if (blob is null) return;
    if (!blob.isMyPlayer()) return;
    if (getHUD().hasMenus()) return; // NOTE(hobey): if you using a shop next the tent/ballista/etc and use numbers to buy things don't class change
    if (!canChangeClass(this, blob)) return;
    if (blob.getTickSinceCreated() <= 1) return; // NOTE(hobey): prevent infinite loop
    ...
    for (uint i = 0 ; i < classes.length; i++)
    {
        PlayerClass @pclass = classes[i];
        CBitStream params;
        write_classchange(params, blob.getNetworkID(), pclass.configFilename);
        
        CControls@ controls = getControls();
        if ((i+1 >= 1) && (i+1 <= 9)) {
            if (controls.isKeyJustPressed(EKEY_CODE(KEY_KEY_1 + i))) {
                this.SendCommand(SpawnCmd::changeClass, params);
            }
        } else if (i+1 == 10) {
            if (controls.isKeyJustPressed(EKEY_CODE(KEY_KEY_0))) {
                this.SendCommand(SpawnCmd::changeClass, params);
            }
        }
    }
