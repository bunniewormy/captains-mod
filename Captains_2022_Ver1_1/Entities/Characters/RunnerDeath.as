...
void StuffFallsOut(CBlob@ this)
{
	...
	while (inv !is null && inv.getItemsCount() > 0)
	{
		...
		if (blob.getName() == "mat_wood" || blob.getName() == "mat_stone")
		{
			string zone = getBlobsZone(this);

			if(zone=="a")
			{
				uint16 quantity = blob.getQuantity();
				uint16 stay_quantity = Maths::Round(0.80f * quantity);
				blob.server_SetQuantity(stay_quantity);
				...
				if(getBlobsByName("tent", blist))
				{
					for(uint step=0; step<blist.length; ++step)
					{
						...
						{
							CBlob@ mat = server_CreateBlobNoInit(blob.getName());
							...
							if (mat !is null)
							{
								...
								mat.server_SetQuantity(quantity - stay_quantity);
								
								mat.setPosition(blist[step].getPosition());
							}
			...
			{
				uint16 quantity = blob.getQuantity();
				uint16 stay_quantity = Maths::Round(0.75f * quantity);
				blob.server_SetQuantity(stay_quantity);
				uint team = this.getTeamNum();
