...
void onInit(CBlob@ this)
{
	...
	this.addCommandID("drop builder mats");

	this.push("names to activate", "keg");
	this.push("names to activate", "superkeg");
	...
	AddIconToken("$DROP_BUILDER_MATS$", "dropmats.png", Vec2f(32, 32), 0);
}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("get bomb"))
	{
		...
		if (hasItem(this, bombTypeName))
		{
			...
			else if (bombType == 2)
			{
				if (getNet().isServer())
				{
					CBlob @blob = server_CreateBlob("teleportbomb", this.getTeamNum(), this.getPosition());
					if (blob !is null && this.getPlayer() !is null)
					{
						TakeItem(this, bombTypeName);
						this.server_Pickup(blob);
	...
	else if (cmd == this.getCommandID("drop builder mats") && isServer())
	{
		u16 id;
		...
		CInventory@ inv = target.getInventory();
		...
		for (int i = 0; i <= inv.getItemsCount(); ++i)
		{
			...
			if (blob.getName() != "mat_wood" && blob.getName() != "mat_stone" && blob.getName() != "mat_gold" && blob.getName() != "drill")
			{
				continue;
			}
		...
		CBlob@ held = target.getCarriedBlob();
		if (held is null) return;

		if (held.getName() != "mat_wood" && held.getName() != "mat_stone" && held.getName() != "mat_gold" && held.getName() != "drill")
		{
			return;
		}
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info)
{
	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		// start from further ones to avoid raycasting through blobs we already hit too many times
		for (int i = hitInfos.size() - 1; i >= 0; i--)
		{
			...
			if (b !is null)
			{
				...
				HitInfo@[] rayInfos;
				// do a raycast for slightly more than hitvec to make sure it reaches the blob it's directed at
				...
				for (int j = 0; j < rayInfos.size(); j++)
				{
					...
					bool isDoor = rayb.hasTag("blocks sword") && !rayb.isAttached() && rayb.isCollidable();
					if (knight_has_hit_actor(this, rayb)) 
					{
						// in case we hit any of these on previous ticks
						if (isDoor) break;
						if (rayb.getName() == "log")
						{
					...
					// print("hit " + rayb.getName() + " for " + temp_damage + " on " + getGameTime());

					if (isDoor)
					{
						break; // prevent raycasting past the door after we hit it
					}
				}
			}
			else  // hitmap
				if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
				{
					...
					bool castle = map.isTileCastle(hi.tile) && getRules().hasTag("slashstone");
					bool moredmgwood = getRules().hasTag("slashwood");
					bool gold = map.isTileGold(hi.tile);
					bool wood = map.isTileWood(hi.tile);
					if (ground || wood || dirt_stone || castle || gold)
					{
						...
						if (dif < 20.0f)
						{
							...
							bool canhittwice = false;
							if (castle && !jab)
							{
								info.tileDestructionLimiter++;
								info.tileDestructionLimiter++;
								canhit = (info.tileDestructionLimiter % 4 == 0);
							}
							else if (wood && moredmgwood && jab)
							{
								canhit = true;
							}
							else if (wood && moredmgwood && !jab)
...
void onCreateInventoryMenu(CBlob@ this, CBlob@ forBlob, CGridMenu @gridmenu)
{
	...
	if (tool !is null)
	{
		...
		CGridButton@ clear = tool.AddButton("$DROP_BUILDER_MATS$", "", this.getCommandID("drop builder mats"), Vec2f(1, 1), params);
		if (clear !is null)
		{
			clear.SetHoverText(getTranslatedString("Drop builder mats\n"));
		}
	}
}
