...
void LoadSprites(CSprite@ this)
{
	...
	if (p !is null)
	{
		...
		{
			ensureCorrectRunnerTexture(this, "knight_nonamed", "KnightNoNamed");
		}
		else 
		{
			switch (armour)
...
void onTick(CSprite@ this)
{
	...
	else if (knight.state == KnightStates::shielding && isShieldEnabled(blob))
	{
		...
		{
			this.SetAnimation(!crouching ? "shield_raised" : "shield_crouched");
	...
	{
		this.SetAnimation(!crouching ? "draw_sword" : "draw_sword_crouched");
	}
	else if (knight.state == KnightStates::sword_drawn)
	{
		...
		{
			this.SetAnimation(!crouching ? "strike_power_ready" : "strike_power_ready_crouched");
			this.animation.frame = 0;
		}
