...
void onTick(CSprite@ this)
{
	...
	if (!blob.hasTag(burning_tag)) //give way to burning anim
	{
		...
		else if (action2 || ((this.isAnimation("strike") || this.isAnimation("strike_slow") || this.isAnimation("strike_fast") || this.isAnimation("chop") || this.isAnimation("chop_slow") || this.isAnimation("chop_fast")) && !this.isAnimationEnded()))
		{
			...
			if (blob.get("hitdata", @hitdata))
			{
				...
				bool faster_pickaxe = getRules().hasTag("faster pickaxe");

				if (hitting_dirt && getRules().getCurrentState() != INTERMISSION && getRules().getCurrentState() != WARMUP)
				{
					attack_anim = faster_pickaxe ? "strike_very_fast" : "strike_slow";
				}
				else if (hitting_wood)
				{
					attack_anim = hitting_built ? "chop_fast" : "chop";

					if(hitting_built)
					{
						string temps = attack_anim;
						attack_anim = faster_pickaxe ? "chop_very_fast" : temps;
					}
				}
				else if(hitting_stone)
				{
					...
					{
						string temps = attack_anim;
						attack_anim = faster_pickaxe ? "strike_very_fast" : temps;
					}
				}
...
void onRender(CSprite@ this)
{
	...
	if (blob.isKeyPressed(key_action1) || this.isAnimation("strike_slow") ||this.isAnimation("strike") || this.isAnimation("strike_fast") || this.isAnimation("chop_slow") || this.isAnimation("chop") || this.isAnimation("chop_fast") || this.isAnimation("strike_very_fast") || this.isAnimation("chop_very_fast"))
	{
