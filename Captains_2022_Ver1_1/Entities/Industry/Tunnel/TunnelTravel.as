...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	if (this.isOverlapping(caller) && this.getHealth() <= (0.9 * this.getInitialHealth()) && caller.getTeamNum() == this.getTeamNum() && player !is null)
	{
		AddIconToken("$fix_tunnel$", "InteractionIcons.png", Vec2f(32, 32), 15);

		Vec2f pagchomp = Vec2f_zero;
		pagchomp.y = Vec2f_zero.y - 8;

		CBitStream params;
		params.write_u16(caller.getNetworkID());

		CButton@ repair = caller.CreateGenericButton("$fix_tunnel$", pagchomp, this, this.getCommandID("repair"), "Repair Tunnel (Health: " + formatFloat(this.getHealth() / this.getInitialHealth(), '0', 4, 2) + "%)\n(50 coins = 10%)", params);

		CShape@ shape = this.getShape();

		if(shape !is null)
		{
			repair.enableRadius = Maths::Max(this.getRadius(), (shape.getWidth() + shape.getHeight()) / 2) * 2;
		}
...
void onTunnelCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("repair") && isServer())
	{
		CBlob@ caller = getBlobByNetworkID(params.read_u16());
		if (caller !is null)
		{
			...
			if(player !is null)
			{
				if (player.getCoins() >= 50)
				{
					...
					this.server_Heal(this.getInitialHealth() * 0.1f * 2);
		...
		for (uint i = 0; i < tunnels.length; i++)
		{
			...
			else
			{
				...
				menu.AddButton(getTravelIcon(this, tunnel), getTranslatedString(getTravelDescription(this, tunnel)), this.getCommandID("travel to"), Vec2f(BUTTON_SIZE, BUTTON_SIZE), params);
				                ...
				                if ((i+1 >= 1) && (i+1 <= 9)) {
                    menu.AddKeyCommand(EKEY_CODE(KEY_KEY_1 + i), this.getCommandID("travel to"), params);
                } else if (i+1 == 10) {
                    menu.AddKeyCommand(KEY_KEY_0,                this.getCommandID("travel to"), params);
                }
			}
		}
	}
