...
void onInit(CBlob@ this)
{
	...
	if (!this.exists("shop button radius"))
	{
		...
		if (shape !is null)
		{
			if (this.getName() == "midpoint5" || this.getName() == "midpoint6")
			{
				this.set_u8("shop button radius", Maths::Max(this.getRadius(), (shape.getWidth() + shape.getHeight())));
			}
			else
			{
		...
		if (inv !is null && isInRadius(this, caller))
		{
			...
			if (tookReqs)
			{
				if (s.spawnNothing)
				{
                    ...
                    string whatwebuying = blobName.replace(" ", "-");
				...
				else
				{
					...
					else
					{
						...
						if (blob !is null)
						{
							...
							if (blob.getName() == "mat_bombs") blob.server_SetQuantity(1);
							bool pickable = blob.getAttachments() !is null && blob.getAttachments().getAttachmentPointByName("PICKUP") !is null;
							if (spawnToInventory)
							{
								bool add_into_stack = false;
								for (int i = 0; i < callerInv.getItemsCount(); i++)
								{
									CBlob@ currentitem = callerInv.getItem(i);
									if (currentitem.getName() == blob.getName())
									{
										if(currentitem.maxQuantity - currentitem.getQuantity() >= blob.getQuantity())
										{
											add_into_stack = true;
										}
									}
								...
								else if (!blob.canBePutInInventory(caller))
								{
...
void addShopItemsToMenu(CBlob@ this, CGridMenu@ menu, CBlob@ caller)
{
	...
	if (this.get(SHOP_ARRAY, @shop_items))
	{
		for (uint i = 0 ; i < shop_items.length; i++)
		{
			...
			if (s_item.blobName == "cock")
			{
				@button = menu.AddTextButton("", Vec2f(2, 1));
