...
bool hasRequirements(CInventory@ inv1, CInventory@ inv2, CBitStream &inout bs, CBitStream &inout missingBs, bool &in inventoryOnly = false, bool sussyBaka = false)
{
	...
	while (!bs.isBufferEnd())
	{
		...
		{
			bool cancel=false;
			if(inv1 !is null && !sussyBaka && (blobName == "mat_wood" || blobName == "mat_stone" || blobName == "mat_gold") && (getRules().getCurrentState() == WARMUP || getRules().getCurrentState() == INTERMISSION))
			{
				CBlob@[] storages;
...
void server_TakeRequirements(CInventory@ inv1, CInventory@ inv2, CBitStream &inout bs, bool sussyBaka=false)
{
	...
	while (!bs.isBufferEnd())
	{
		...
		if (req == "blob")
		{
			if(inv1.getBlob() !is null && !sussyBaka && (blobName == "mat_wood" || blobName == "mat_stone" || blobName == "mat_gold") && (getRules().getCurrentState() == WARMUP || getRules().getCurrentState() == INTERMISSION))
			{
				CBlob@[] storages;
