...
shared class CTFSpawns : RespawnSystem
{
	...
	void AddPlayerToSpawn(CPlayer@ player)
	{
		...
		string last_death_zone = getRules().get_string("last death zone " + player.getUsername());

		if (last_death_zone == "a")
		{
			tickspawndelay += 60;
		}
		...
		if(rules.isMatchRunning() && ticketsRemaining(rules, teamNum)<=0)
		{
		...
		if (info.team < CTF_core.teams.length)
		{
			...
			printf("amogussy " + info.can_spawn_time);
...
shared class CTFCore : RulesCore
{
	...
	void Update()
	{
		...
		if(ticksToStart <= 40 * getTicksASecond() && (rules.isWarmup()))
		{
			rules.Tag("ctfgivespawnitems_amogus");
		}
	...
	void SetupBases()
	{
        ...
        if(map.hasTag("nomidshops")) hasmidshop = true;
...
{
	if(getGameTime() > 1 && getGameTime() < 90)
	{
		getMap().Untag("nomidshops");
	}
}
