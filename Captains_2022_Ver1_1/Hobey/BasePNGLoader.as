...
class PNGLoader
{
        ...
        {
            {
                ...
                rules.set_s32("team0_expensive_zone_x0", 0);
                rules.set_s32("team0_expensive_zone_x1", 0);
                rules.set_s32("team1_expensive_zone_x0", 0);
                rules.set_s32("team1_expensive_zone_x1", 0);
            }
            ...
            
            {
                ...
                // NOTE(hobey): sync the expensive zone borders
                ...
                // print("sending zonepos command in BasePNGLoader.as: "+team0_x0+","+team0_x1+","+team1_x0+","+team1_x1); // TODO(hobey): removeme
                ...
                /*
                // NOTE(hobey): because this function can execute before onInit in ExpensiveZone;
                // which could cause the zonepos command id to not be set yet in onInit
                if (!rules.hasCommandID("zonepos")) {
    ...
    void handlePixel(const SColor &in pixel, int offset)
    {
        ...
        else
        {
            switch (pixel.color)
            {
                ...
                case map_colors::controlpoint:         autotile(offset); spawnBlob(map, "controlpoint", offset, 255); break;
                case map_colors::midshop4:         autotile(offset); spawnBlob(map, "midshop4", offset, 255); map.Tag("nomidshops"); break;
                case map_colors::midshop5:         autotile(offset); spawnBlob(map, "midshop5", offset, 255); map.Tag("nomidshops"); break;
                case map_colors::dontspawnmidshops:         autotile(offset); map.Tag("nomidshops"); break;
    ...
    void SetupBackgrounds()
    {
        ...
        // background
        Vec2f off(0, -75.0f);
        f32 bg_move_modifier_y = 1.0f;
        map.AddBackground("Sprites/Back/BackgroundPlains.png", Vec2f(0.0f, -18.0f) + off, Vec2f(0.3f, 0.3f * bg_move_modifier_y), color_white);
        map.AddBackground("Sprites/Back/BackgroundTrees.png", Vec2f(0.0f,  -5.0f) + off, Vec2f(0.4f, 0.4f * bg_move_modifier_y), color_white);
        map.AddBackground("Sprites/Back/BackgroundIsland.png", Vec2f(0.0f, 0.0f) + off, Vec2f(0.6f, 0.6f * bg_move_modifier_y), color_white);
        //map.AddBackground("Sprites/Back/BackgroundCastle.png", Vec2f(0.0f, -45.0f) + off, Vec2f(0.7f, 0.7f * bg_move_modifier_y), color_white);
