...
void HealBlob(CBlob@ this, CBlob @healBlob, CBlob @carryBlob, Vec2f cursorPos)
{
    if (healBlob !is null)
    {
        if (!serverBlobCheck(this, healBlob, cursorPos, true))
            return;
...
bool serverBlobCheck(CBlob@ blob, CBlob@ blobToPlace, Vec2f cursorPos, bool healing=false)
{
    ...
    if (blobAtPos !is null)
    {
        ...
        else if (blobAtPos.getName() == blobToPlace.getName() && !healing && blobAtPos.getName() != "ladder")
        {
            return false;
        }
    }
        ...
        if (!getHUD().hasButtons() && !carryBlob.hasTag("custom drop"))
        {
            if (this.isKeyPressed(key_action1))
            {
                if (snap && bc.cursorClose && bc.hasReqs && bc.buildable && bc.supported)
                {
                    ...
                    if (blobAtPos !is null && carryBlob.getConfig() == blobAtPos.getConfig() && blobAtPos.getHealth() < blobAtPos.getInitialHealth() && blobAtPos.getName() != "ladder")
                    {
                        CBitStream params;
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
    ...
    if (cmd == this.getCommandID("healBlob"))
    {
        ...
        if (carryBlob !is null && healBlob !is null)
        {
