...
void onInit(CInventory@ this)
{
    ...
    blob.set_TileType("buildtile", 0); CBitStream params; params.write_u16(0); blob.SendCommandOnlyServer(blob.getCommandID("sync buildtile"), params);
...
void MakeBlocksMenu(CInventory@ this, const Vec2f &in INVENTORY_CE)
{
    ...
    if (menu !is null)
    {
        ...
        for(u8 i = 0; i < blocks[PAGE].length; i++)
        {
            ...
            if (getRules().hasTag("disablestone"))
            {
                if (b.name.find("stone") != -1 && b.name.find("back") == -1)
                {
                    CGridButton@ button = menu.AddButton(b.icon, "Disable stone blocks modifier enabled", Builder::make_block + i);
            ...
            bool has_recs = false;
            CBitStream requirements; getZoneRequirements(blob, b.reqs, true, requirements, b.name);

            if (b.buildOnGround) 
            {
                if (hasRequirements(this, requirements, missing, not b.buildOnGround))
                {
                    has_recs = true;
                }
            } 
            ...
            if (has_recs) 
            {
                bool has_exp_recs = false;
                string string1 = block_desc + "\nNormal cost:" + getButtonRequirementsText(b.reqs, false);
                if (hasRequirements(this, requirements, missing, not b.buildOnGround)) 
                {
                    has_exp_recs = true;
                }
                button.hoverText = block_desc + "\nNormal cost:" + getButtonRequirementsText(b.reqs, false) + "\nCurrent cost:" + getButtonRequirementsText(requirements, !has_exp_recs);
            }
            else
            {
                bool has_exp_recs = false;
                string string1 = block_desc + "\nNormal cost:" + getButtonRequirementsText(missing, true);
                if (hasRequirements(this, requirements, missing, not b.buildOnGround)) 
                {
                    has_exp_recs = true;
                }
                button.hoverText = string1 + "\nCurrent cost:" + getButtonRequirementsText(missing, !has_exp_recs);
                button.SetEnabled(false);
            }
...
void onCommand(CInventory@ this, u8 cmd, CBitStream@ params)
{
    ...
    if (cmd >= Builder::make_block && cmd < Builder::make_reserved)
    {
        ...
        if (blocks !is null && i >= 0 && i < blocks[PAGE].length)
        {
            ...
            {
                blob.set_TileType("buildtile", block.tile); CBitStream params; params.write_u16(block.tile); blob.SendCommandOnlyServer(blob.getCommandID("sync buildtile"), params);
            }
...
void onRender(CSprite@ this)
{
    ...
    if (localBlob is blob)
    {
        ...
        // show cant build
        if (blob.isKeyPressed(key_action1) || blob.get_u32("show build time") + 90 > getGameTime())
        {
            ...
            if (bc !is null)
            {
                ...
                Vec2f boxpos( myPos.x, myPos.y - 100.0f );
                const string missingText = getButtonRequirementsText( bc.missing, true );
                string bubu = missingText;

                bool inzoneb = false;

                if (bubu.findFirst("9999", 0) != -1)
                {
                    inzoneb = true;
                    GUI::DrawText( getTranslatedString("Cannot build stone blocks in this zone"), Vec2f(boxpos.x - 50, boxpos.y - 15.0f), Vec2f(boxpos.x + 50, boxpos.y + 15.0f), color_black, false, false, true );
                }
