...
void PlaceBlock(CBlob@ this, u8 index, Vec2f cursorPos)
{
    ...
    CBitStream missing;
    CBitStream requirements; getZoneRequirements(this, bc.reqs, false, requirements, bc.name);
    
    CInventory@ inv = this.getInventory();

    bool validTile = bc.tile > 0;
    bool hasReqs = hasRequirements(inv, requirements, missing);
    bool passesChecks = serverTileCheck(this, index, cursorPos);

    if (validTile && hasReqs && passesChecks)
    {
        server_TakeRequirements(inv, requirements/*bc.reqs*/);
...
bool serverTileCheck(CBlob@ blob, u8 tileIndex, Vec2f cursorPos)
{
    ...
    if (map.getTile(cursorPos).type == blockToPlace.tile)
    {
        return false;
    }
    ...
    if (blobAtPos !is null && map.isTileSolid(blockToPlace.tile) && (
        ...
        blobAtPos.getConfig() == "wooden_platform" || 
        blobAtPos.getConfig() == "ladder" || 
        blobAtPos.getConfig() == "bridge"))
    {
        return false;
    }
...
void onTick(CBlob@ this)
{
    ...
    if (buildtile > 0)
    {
        ...
        {
            bc.missing.Clear();
            CBitStream requirements; getZoneRequirements(this, block.reqs, false, requirements, block.name);
            bc.hasReqs = hasRequirements(this.getInventory(), requirements, bc.missing, not block.buildOnGround);
        }
        ...
        {
            if (bc.cursorClose && bc.buildable && bc.supported && bc.hasReqs)
            {
                CBitStream params;
...
void onRender(CSprite@ this)
{
    ...
    if (buildtile > 0)
    {
        ...
        if (localBlob is blob) {
            ...
            Vec2f held_tile_pos = blob.getPosition();
            held_tile_pos.y -= 5.f;
            if (blob.isFacingLeft()) {
                held_tile_pos.x -= 8.f;
            }
            
            SColor color; color.set(255, 255, 255, 255);
            map.DrawTile(held_tile_pos, buildtile, color, getCamera().targetDistance, false);
        }
    }
}
