...
void onInit(CBlob@ this)
{
    ...
    this.addCommandID("give server tileAimPos");
	this.addCommandID("hitdata sync");
	this.addCommandID("sync buildtile");
...
void onTick(CBlob@ this)
{
	...
	if (!ismyplayer) {
        if (this.exists("buildtile_syncing")) {
            this.set_TileType("buildtile", this.get_u16("buildtile_syncing"));
        }
    }
	...
	// NOTE(hobey): sync bc.tileAimPos; i don't trust this.Sync() right now, i'm gonna use a command instead
    ...
    if (player !is null) {
        if (ismyplayer) {
            if (!getNet().isServer()) {
                ...
                if (bc is null) {
                    warn("client[" + player.getUsername()+"]: bc null");
                } else {
                    // print("client[" + player.getUsername()+"]: syncing bc.tileAimPos to server");
                    CBitStream params;
                    params.write_Vec2f(bc.tileAimPos);
                    this.SendCommandOnlyServer(this.getCommandID("give server tileAimPos"), params);
                }
            }
        }
        /*
        if (getNet().isServer()) {
            ...
            if (bc is null) {
                print("server[" + player.getUsername()+"]: bc null");
            } else {
                print("server[" + player.getUsername()+"]: x" + bc.tileAimPos.x);
            }
        } else {
            BlockCursor @bc;
            this.get("blockCursor", @bc);
            ...
            if (bc is null) {
                print("client[" + player.getUsername()+"]: bc null");
            } else {
                print("client[" + player.getUsername()+"]: x" + bc.tileAimPos.x);
            }
        }
        */
    }
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("give server tileAimPos")) {
        if (getNet().isServer()) {
            BlockCursor @bc;
            this.get("blockCursor", @bc);
            if (bc is null) {
                // warn("server onCommand: bc is null");
            } else {
                bc.tileAimPos = params.read_Vec2f();
            }
        }
    }
    else if (cmd == this.getCommandID("sync buildtile")) {
        u16 buildtile = params.read_u16();
        if (getNet().isServer()) {
            // this.set_TileType("buildtile", buildtile);
            this.set_u16("buildtile_syncing", buildtile);
            this.Sync("buildtile_syncing", true);
            
            // CBitStream new_params;
            // new_params.write_u16(buildtile);
            // this.SendCommand(this.getCommandID("sync buildtile"), new_params);
            // } else {
            // if (!this.isMyPlayer()) {
            // this.set_TileType("buildtile", buildtile);
            // }
...
void Pickaxe(CBlob@ this)
{
	...
	bool strikeAnim = sprite.isAnimation("strike_slow") || sprite.isAnimation("strike") || sprite.isAnimation("strike_fast") || sprite.isAnimation("chop_slow") || sprite.isAnimation("chop") || sprite.isAnimation("chop_fast") || sprite.isAnimation("strike_very_fast") || sprite.isAnimation("chop_very_fast");
...
void onDetach(CBlob@ this, CBlob@ detached, AttachmentPoint@ attachedPoint)
{
    ...
    if (detached.hasTag("temp blob"))
    {
        ...
        if (i >= 0 && i < blocks[PAGE].length)
        {
            ...
            {
                this.set_u8("buildblob", 255);
                this.set_TileType("buildtile", 0); CBitStream params; params.write_u16(0); this.SendCommandOnlyServer(this.getCommandID("sync buildtile"), params);
                
                CInventory@ inv = this.getInventory();
                
                CBitStream missing;
                // print("builder logic blob detached; pos: "+pos.x+", "+pos.y);
                CBitStream requirements; getZoneRequirements(this, b.reqs, false, requirements, b.name);
                if (hasRequirements(inv, requirements, missing, not b.buildOnGround))
                {
                    server_TakeRequirements(inv, requirements);
                }
